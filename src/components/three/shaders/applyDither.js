...
export function applyDither(material, { strength = 1, levels = 6, pixelSize = 6 } = {}) {
  material.onBeforeCompile = (shader) => {
    shader.uniforms.uDitherStrength = { value: strength };
    shader.uniforms.uDitherLevels = { value: levels };
    shader.uniforms.uDitherPixelSize = { value: pixelSize };
    ...
    shader.fragmentShader = shader.fragmentShader
      ...
      .replace(
        ...
        if (uDitherStrength > 0.0) {
          gl_FragColor.rgb = ditherQuantize(gl_FragColor.rgb, gl_FragCoord.xy, uDitherLevels, uDitherStrength, uDitherPixelSize);
        }`,
      );

    material.userData.shader = shader;
  };
  material.customProgramCacheKey = () => `dither-${strength}-${levels}-${pixelSize}`;
}
